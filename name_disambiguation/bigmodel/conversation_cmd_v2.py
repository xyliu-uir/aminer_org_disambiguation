...
def prepare_print_diff(nextStr: Callable[[any], str], printError: Callable[[], None]):
    ...
    def print_diff(input):
        nonlocal previous
        str = nextStr(input)
        # str = "chinese academy sciences, changchun institute apply chemicalstate keylaboratorvpolymer physics and chemical, changchun 130022, peoples r china. 提取'一级机构','二级机构','地理位置','城市','国家',并输出JSON格式"
...
if __name__ == "__main__":
    ...
    if token_result and token_result["code"] == 200:
        ...
        while (True):
            ...
            promp = "提取'一级机构','二级机构','地理位置','城市','国家',并输出JSON格式"
